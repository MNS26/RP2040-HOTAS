...
class Status(Thread):
  def __init__(self):
    ...
    self.monitoring_thread = Thread(target=self.monitor_file_changes,name="Status.json Monitor")
  ...
  def monitor_file_changes(self):
    while self.is_running:
      ...
      file = os.path.join(str(self.directory),'Status.json') 
      if file:
        print(f"Accessing latest Status.json")
        with open(file, "r") as Status:
          self.Status = json.load(Status)
          print(self.Status)
      else:
        print(f"No Status.json found in: {self.directory}")  
        pass
