''' app/utils/monitor/modulesinfo.py '''
...
class ModulesInfo(Thread):
  def __init__(self):
    self.AppConfig = AppConfig()
    self.directory = self.AppConfig.game_log_dir
    self.monitoring_thread = Thread(target=self.monitor_file_changes,name="Shipyard Monitor")
  ...
  def monitor_file_changes(self):
    while self.is_running:
      ...
      ShipyardFile = os.path.join(str(self.directory),'Shipyard.json') 

      if ShipyardFile:
        print(f"Accessing latest Shipyard.json")
      else:
        print(f"No Shipyard.json found in: {self.directory}")  
        pass
