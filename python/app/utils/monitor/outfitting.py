''' app/utils/monitor/outfitting.py '''
...
class Outfitting(Thread):
  def __init__(self):
    self.AppConfig = AppConfig()
    self.directory = self.AppConfig.game_log_dir
    self.monitoring_thread = Thread(target=self.monitor_file_changes,name="Outfitting.json Monitor")
  ...
  def monitor_file_changes(self):
    while self.is_running:
      ...
      file = os.path.join(str(self.directory),'Outfitting.json') 

      if file:
        print(f"Accessing latest Outfitting.json")
      else:
