...
uint16_t USBD_MSC::getInterfaceDescriptor(uint8_t itfnum_deprecated,
                                                   uint8_t *buf,
                                                   uint16_t bufsize) {
  ...
  uint8_t const itfnum = USBDevice.allocInterface(1);
  uint8_t const ep_in = USBDevice.allocEndpoint(TUSB_DIR_IN);
  uint8_t const ep_out = USBDevice.allocEndpoint(TUSB_DIR_OUT);

  uint8_t const desc[] = {
      TUD_MSC_DESCRIPTOR(itfnum, stringIndex, ep_out, ep_in, BULK_PACKET_SIZE)};
...
uint8_t USBD_MSC::setStringIndex(uint8_t index) {
  if (index >= 12)
  return 0;
...
void USBD_MSC::setStartStopCallback(uint8_t lun,
...
void USBD_MSC::setWritableCallback(uint8_t lun,
