...
void USBD_Device::setSerialDescriptor(const char *s) {
  _desc_str_arr[STRID_SERIAL] = s;
}

// Add a string descriptor to the device's pool
// Return string index
uint8_t USBD_Device::addStringDescriptor(const char* s) {
...
bool USBD_Device::begin(uint32_t rhport) {
  clearConfiguration();
...
bool USBD_Device::isInitialized(uint8_t rhport) {
  (void)rhport;
  return tud_inited();
}
...
board_usb_get_serial(_desc_str + 1, 32);
