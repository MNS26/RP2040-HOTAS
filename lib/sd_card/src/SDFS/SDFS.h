...
class SDFSFileImpl;
class SDFSDirImpl;
class SDFSConfug : public FSConfig {
public:
  static constexpr uint32_t FSId = 0x53444653;
  SDFSConfig(uint8_t csPin = 4, uint32_t spi = SD_SCK_MHZ(10), spi_hw_t &port = SPI) : FSConfig(FSId, false), _csPin(csPin), _part(0), _spiSettings(spi), _spi(&port)  { }
